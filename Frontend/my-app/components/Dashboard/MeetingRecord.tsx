...
const MeetingRecord = () => {
    ...
    const [activeMode, setActiveMode] = useState<'record' | 'audio' | 'transcript' | null>('record');
        ...
        <>
            ...
            {!recorder.isRecording && (
                <div className={styles.container}>
                    ...
                    <div className={styles.breadcrumbs}>PROJECTS &gt; BLUENOTE &gt; ACTIVE SESSION</div>
                    <h1 className={styles.title}>{dict.record.title}</h1>

                    {/* ── Workflow Mode Selector ── */}
                    <div className={styles.workflowSelector}>
                        <button
                            className={`${styles.workflowCard} ${activeMode === 'record' ? styles.workflowCardActive : ''}`}
                            onClick={() => setActiveMode(activeMode === 'record' ? null : 'record')}
                        >
                            <div className={`${styles.workflowCardIcon} ${activeMode === 'record' ? styles.workflowCardIconActive : ''}`}>
                            ...
                            <div>
                                <div className={styles.workflowCardLabel}>Record Live</div>
                                <div className={styles.workflowCardDesc}>Capture audio from microphone in real-time</div>
                            </div>
                            <div className={`${styles.workflowCardChevron} ${activeMode === 'record' ? styles.workflowCardChevronOpen : ''}`}>
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="6 9 12 15 18 9"/></svg>
                            </div>
                        </button>

                        <button
                            className={`${styles.workflowCard} ${activeMode === 'audio' ? styles.workflowCardActive : ''}`}
                            onClick={() => setActiveMode(activeMode === 'audio' ? null : 'audio')}
                        >
                            <div className={`${styles.workflowCardIcon} ${activeMode === 'audio' ? styles.workflowCardIconActive : ''}`}>
                                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            ...
                            <div>
                                <div className={styles.workflowCardLabel}>Upload Audio File</div>
                                <div className={styles.workflowCardDesc}>MP3, WAV, M4A — up to 25 MB</div>
                            </div>
                            {uploadedAudioFiles.length > 0 && (
                                <span className={styles.workflowCardBadge}>{uploadedAudioFiles.length}</span>
                            )}
                            <div className={`${styles.workflowCardChevron} ${activeMode === 'audio' ? styles.workflowCardChevronOpen : ''}`}>
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="6 9 12 15 18 9"/></svg>
                            </div>
                        </button>

                        <button
                            className={`${styles.workflowCard} ${activeMode === 'transcript' ? styles.workflowCardActive : ''}`}
                            onClick={() => setActiveMode(activeMode === 'transcript' ? null : 'transcript')}
                        >
                            <div className={`${styles.workflowCardIcon} ${activeMode === 'transcript' ? styles.workflowCardIconActive : ''}`}>
                                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            ...
                            <div>
                                <div className={styles.workflowCardLabel}>Upload Transcript</div>
                                <div className={styles.workflowCardDesc}>TXT, MD, DOCX — generate MOM directly</div>
                            </div>
                            {uploadedTranscribeFiles.length > 0 && (
                                <span className={styles.workflowCardBadge}>{uploadedTranscribeFiles.length}</span>
                            )}
                            <div className={`${styles.workflowCardChevron} ${activeMode === 'transcript' ? styles.workflowCardChevronOpen : ''}`}>
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="6 9 12 15 18 9"/></svg>
                            </div>
                        </button>
                    </div>
                    ...
                    {activeMode === 'record' && (
                        <div className={styles.workflowPanel}>
                            <div className={styles.panelSection}>
                                <div className={styles.panelLabel}>AUDIO MODE</div>
                                <div className={styles.modeToggleGroup}>
                                    <button className={`${styles.modeToggleBtn} ${!systemAudioMode ? styles.modeToggleBtnActive : ''}`} onClick={() => setSystemAudioMode(false)}>
                                    ...
                                    <button className={`${styles.modeToggleBtn} ${systemAudioMode ? styles.modeToggleBtnActive : ''}`} onClick={() => setSystemAudioMode(true)}>
                            ...
                            <div className={styles.panelSection}>
                                <div className={styles.panelLabel}>MICROPHONE</div>
                                <div className={styles.micRow}>
                                    <button onClick={recorder.toggleMicMute} className={`${styles.iconBtn} ${recorder.isMicMuted ? styles.iconBtnDanger : ''}`} title={recorder.isMicMuted ? 'Unmute' : 'Mute'}>
                                        {recorder.isMicMuted ? (
                                    ...
                                    <select className={styles.deviceSelectInline} value={recorder.selectedDeviceId} onChange={(e) => recorder.setSelectedDeviceId(e.target.value)}>
                                        {recorder.devices.map(device => (
                                            <option key={device.deviceId} value={device.deviceId}>
                                                {device.label || `Microphone ${device.deviceId.slice(0, 5)}`}
                                            </option>
                            ...
                            <div className={styles.panelRecordCenter}>
                                ...
                                <div className={styles.panelHint}>Click to start recording</div>
                            </div>
                        </div>
                    )}
                    ...
                    {activeMode === 'audio' && (
                        <div className={styles.workflowPanel}>
                            <input type="file" ref={audioFileInputRef} style={{ display: 'none' }} multiple accept=".mp3,.wav,.m4a" onChange={handleAudioFileChange} />
                            <div className={styles.uploadArea} onClick={handleAudioUploadClick} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleAudioDrop}>
                                {uploadedAudioFiles.length === 0 ? (
                                    <>
                                        ...
                                        <div className={styles.uploadText}>Drop audio files here or click to browse</div>
                                        <div className={styles.uploadSubtext}>Supports MP3, WAV, M4A — max 25 MB per file</div>
                                    </>
                                ) : (
                                        ...
                                        {uploadedAudioFiles.map((file, idx) => (
                                            <div key={idx} className={styles.uploadedFileRow} onClick={(e) => e.stopPropagation()}>
                                                <div className={styles.uploadedFileInfo}>
                                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6366f1" strokeWidth="2"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/></svg>
                                                ...
                                                <div className={styles.uploadedFileActions}>
                                                    <button className={styles.transcribeFileBtn} onClick={(e) => { e.stopPropagation(); handleTranscribeUploadedAudio(file); }}>Process</button>
                                                    <button className={styles.removeFileBtn} onClick={(e) => { e.stopPropagation(); removeAudioFile(idx); }}>
                                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                                                    </button>
                                                </div>
                                            </div>
                                        ))}
                                        <div className={styles.uploadSubtext} style={{ marginTop: '0.5rem' }}>Click to add more files</div>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {/* ── Panel: Upload Transcript ── */}
                    {activeMode === 'transcript' && (
                        <div className={styles.workflowPanel}>
                            <input type="file" ref={transcribeFileInputRef} style={{ display: 'none' }} multiple accept=".txt,.md,.docx" onChange={handleTranscribeFileChange} />
                            <div className={styles.uploadArea} onClick={handleTranscribeUploadClick} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleTranscribeDrop}>
                                {uploadedTranscribeFiles.length === 0 ? (
                                    <>
                                        <div className={styles.uploadIcon}>
                                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#6366f1" strokeWidth="2.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
                                        </div>
                                        <div className={styles.uploadText}>Drop transcript here or click to browse</div>
                                        <div className={styles.uploadSubtext}>Supports TXT, MD, DOCX</div>
                                    </>
                                ) : (
                                    <div className={styles.uploadAreaFiles}>
                                        {uploadedTranscribeFiles.map((file, idx) => (
                                            <div key={idx} className={styles.uploadedFileRow} onClick={(e) => e.stopPropagation()}>
                                                ...
                                                <div className={styles.uploadedFileActions}>
                                                    <button className={styles.removeFileBtn} onClick={(e) => { e.stopPropagation(); removeTranscribeFile(idx); }}>
                                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                                                    </button>
                                                </div>
                                            </div>
                            ...
                            {uploadedTranscribeFiles.length > 0 && (
                                <button className={styles.proceedBtn} onClick={handleGenerateDocx} style={{ width: '100%', marginTop: '16px' }}>
                    ...
                    <div className={styles.saveModal}>
                        ...
                        <div className={styles.saveModalContent}>
                            ...
                            {transcript.transcriptResult && (
                                <div className={styles.transcriptPanel}>
                                    <div className={styles.transcriptPanelHeader}>
                                        ...
                                        <button onClick={handleDownloadTranscript} className={styles.transcriptDownloadBtn} title="Download .txt">
                            ...
                            <div className={styles.saveActions}>
                                ...
                                <button className={styles.btnDownload} onClick={handleDownloadMP3} title={dict.record.downloadMp3 || 'Download MP3'}>
